...
export async function POST(req: NextRequest) {
  try {
    ...
    if (profile) {
      ...
      const token = jwt.sign({ id: profile._id, type }, process.env.JWT_SECRET!, { expiresIn: '1h' })
      const resp: NextResponse = NextResponse.json({ message: "Profile found", profile, token }, { status: 200 })
